...
//  scheckv3
//
//  Created by Dhruv Ranjan, Kunal Thacker on 7/17/15.
...
class ManagerSignupViewController: UIViewController,UITextFieldDelegate  {
        ...
        @IBAction func doneButton(sender: AnyObject) {
            if (nameTextField.text.isEmpty || emailTextField.text.isEmpty ||
                ...
                if empItems.count == 0 {
                    ...
                    saveNewItem(idTextField.text, companyName: companyTextField.text, email: emailTextField.text, fullTime: "0", partTime: "0", password: passwordTextField.text)
                    fetchLog()
        ...
        func hasSignedUp(String) -> Bool {
            ...
            if empItems.count == 0 {
                ...
                while (i < empItems.count) {
                    if empItems[i].password.isEmpty || empItems[i].companyEmail.isEmpty {
                        return false
                    }
                    i = i + 1
                }
    ...
    func saveNewItem(id : String, companyName : String, email : String, fullTime : String, partTime : String, password : String) {
        
            // Create the new  log item
            var newLogItem = CompanySettings.createInManagedObjectContext(self.managedObjectContext!, companyID: id, companyName: companyName, fullTime: fullTime, partTime: partTime, companyEmail: email, password: password)
            save()
        }
        ...
        func fetchLog() {
            ...
            let pred = NSPredicate(format: "(companyID = %@)", idTextField.text)
        ...
        var kPreferredTextFieldToKeyboardOffset: CGFloat = 300
